...
test("Stale Host snapshots are rejected atomically", () => { const f=fixture(); const original=structuredClone(f.room.gameState); action(f,"addOmenToBag"); rejects(()=>f.manager.checkState("host-socket",{gameState:original,baseVersion:0}),"STALE_STATE"); assert.equal(f.room.gameState.bag.omen,1); });
...
test("Host-client Check commits without server rolling and keeps snapshot locks", () => { const f=fixture(); action(f,"setAct",["Act 1"]); const local=State.createStore({storage:null,initialState:f.room.gameState}); local.drawCheck({aspect:"Courage",baseTn:7,forcedOmen:true},()=>0); const drawn=local.getState(); f.manager.checkState("host-socket",{gameState:drawn,baseVersion:f.room.gameVersion}); assert.equal(f.room.gameState.currentCheck.originalRoll,null); assert.throws(()=>action(f,"setAct",["Act 3"])); const invalid=structuredClone(drawn); invalid.currentCheck.act="Act 3"; rejects(()=>f.manager.checkState("host-socket",{gameState:invalid,baseVersion:f.room.gameVersion}),"INVALID_STATE"); local.rollCheck(()=>1); local.takeWound(); f.manager.checkState("host-socket",{gameState:local.getState(),baseVersion:f.room.gameVersion}); assert.equal(f.room.gameState.characters[0].wounds,1); });
