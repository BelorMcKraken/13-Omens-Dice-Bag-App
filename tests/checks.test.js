...
function fixture({ wounds = [0,0], bagOmen = 0, strain = {}, autoStrain = false, act = "Act 2", drawLast = false, faces = [1,1,1,1,1,1,1,1] } = {}) {
  ...
  const game=structuredClone(room.gameState); game.act=act; game.bag.omen=bagOmen;
  ...
  const base={characterId:game.characters[0].id,aspect:"Courage",rating:"Average",manualTn:false,allowPlayerRating:true,baseTn:7,difficultyModifier:0,edges:0,flaws:0,risky:false,harmless:false,forcedOmen:false};
  const call=(extra={})=>manager.checks.handle("h","check:create",{baseVersion:room.gameVersion,configuration:{...base,...extra}});
...
test("Known Player Rating updates TN but Host conditions stay immutable",()=>{const f=fixture();f.call({difficultyModifier:1,edges:1,risky:true});f.send("check:set-rating",{rating:"Good"});assert.equal(check(f).finalTn,6);assert.equal(check(f).configuration.edges,1);assert.equal(check(f).configuration.risky,true);rejection(()=>f.send("check:set-rating",{rating:"Great",difficultyModifier:-2}),"INVALID_PAYLOAD");});
test("Fake Rating and arbitrary base TN rejected",()=>{const f=fixture();f.call();rejection(()=>f.send("check:set-rating",{rating:"Perfect"}),"INVALID_PAYLOAD");rejection(()=>f.send("check:set-rating",{rating:"Good",baseTn:1}),"INVALID_PAYLOAD");});
...
test("Host can lock a nonmanual Rating",()=>{const f=fixture();f.call({allowPlayerRating:false});rejection(()=>f.send("check:set-rating",{rating:"Good"}),"RATING_LOCKED");});
...
test("Fourth Wound returns only dead character's Omens",()=>{const f=fixture({wounds:[3,2,1,1,0,0]});f.call({forcedOmen:true});f.send("check:draw");f.send("check:roll");f.send("check:take-wound");assert.equal(f.room.gameState.characters[0].active,false);assert.deepEqual(f.room.gameState.characters.slice(1).map(c=>c.wounds),[2,1,1,0,0]);assert.equal(f.room.gameState.bag.omen,4);f.invariant();});
...
test("Duplicate reroll and Wound resolution rejected",()=>{const f=fixture();f.call({forcedOmen:true});f.send("check:draw");f.send("check:roll");f.send("check:reroll");rejection(()=>f.send("check:reroll"),"ILLEGAL_ACTION");f.send("check:take-wound");rejection(()=>f.send("check:take-wound"),"NO_PENDING_CHECK");f.invariant();});
...
test("Real Host and two Players receive identical server Check phases and results",async t=>{
  ...
  const result=await send(h,"check:create",{baseVersion:1,configuration:{characterId:id,aspect:"Courage",rating:"Average",manualTn:false,allowPlayerRating:true,edges:0,flaws:0,difficultyModifier:1,risky:true,harmless:false,forcedOmen:true}});
  assert.equal(result.ok,true);
  let version=result.room.gameVersion;const checkId=result.room.gameState.currentCheck.id;
  for(const [event,extra] of [["check:set-rating",{rating:"Good"}],["check:draw",{}],["check:roll",{}],["check:take-wound",{}]]){
